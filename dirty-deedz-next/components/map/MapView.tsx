...
export default function MapView({
  ...
  // Native touch handlers — iOS Safari won't honour touch-action:pan-x when a
  // vertically-scrollable ancestor exists. Non-passive touchmove lets us call
  // preventDefault() to claim the gesture before the page scroll wins.
  useEffect(() => {
    const el = innerGridRef.current;
    if (!el || !cardView) return;
    ...
    const onTouchEnd = () => {
      ...
      const count = Math.max(pagedPins.length, 1);
      const cardWidth = el.scrollWidth / count;
      const idx = Math.max(0, Math.min(Math.round(el.scrollLeft / cardWidth), count - 1));
